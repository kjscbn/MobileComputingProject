/* 
  utility functions to read and write string messages according to the
  hopefully human protocol.  These are just raw strings containing JSON
  the only "trick" is that request messages must be terminated by an EOT
  character so the server knows to reverse the stream
*/
